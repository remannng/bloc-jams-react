...
/* WEBPACK VAR INJECTION */(function(fetch, Promise) {/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "onStartup", function() { return onStartup; });
...
var collectionId = 317099; // Unsplash's curated collection
...
function onSearchPhoto(context) {
  var searchTerm = UI.getStringFromUser('Search Unsplash for…', 'People').trim();
...
function setImageForContext(context) {
  ...
  items.forEach(function (item, index) {
    return setImageFor.apply(void 0, [item, index, dataKey].concat(params));
  });
}
...
function setImageFor(item, index, dataKey, searchTerm, photoId) {
  ...
  fetch(url, apiOptions).then(function (response) {
    ...
    return process(json, dataKey, index, item);
  }).catch(function (e) {
    UI.message(e);
    console.error(e);
  });
...
function process(data, dataKey, index, item) {
  // supply the data
  return getImageFromURL(data.urls.regular).then(function (imagePath) {
    if (!imagePath) {
    ...
    var downloadLocation = data.links.download_location + '?client_id=' + API_KEY;
    return fetch(downloadLocation, apiOptions).then(UI.message('📷 by ' + data.user.name + ' on Unsplash'));
  });
}
...
function getImageFromURL(url) {
  ...
  .then(saveTempFileFromImageData).catch(function (err) {
    console.error(err);
...
/* WEBPACK VAR INJECTION */}.call(this, __webpack_require__(/*! ./node_modules/sketch-polyfill-fetch/lib/index.js */ "./node_modules/sketch-polyfill-fetch/lib/index.js"), __webpack_require__(/*! ./node_modules/promise-polyfill/lib/index.js */ "./node_modules/promise-polyfill/lib/index.js")))
...
that['[object Object]'] = __skpm_run.bind(this, '[object Object]');
that['onRun'] = __skpm_run.bind(this, 'default');
that['onStartup'] = __skpm_run.bind(this, 'onStartup');
that['onShutdown'] = __skpm_run.bind(this, 'onShutdown');
that['onSupplyRandomPhoto'] = __skpm_run.bind(this, 'onSupplyRandomPhoto');
that['onSearchPhoto'] = __skpm_run.bind(this, 'onSearchPhoto')
